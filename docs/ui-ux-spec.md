...
### Time-aware home
- Greeting + date; quiet.
- Optional pacing line when recovery strain is elevated (soft, neutral card with
  an info-coloured icon): "Recovery looked lower last night — a gentler day
  might help." Never suggests more activity.
- One primary action, contextual to time of day: morning → "Morning check-in
  (~20s)"; evening → "Evening check-in"; done → a calm "all logged today".
- "Same as yesterday" one-tap shortcut directly under the primary action.
- "What we're watching": at most one or two insight cards (see anatomy below).
...
- Confidence pill (top-left) + `n` (top-right).
- One plain sentence stating the association.
...
- **One-tap quick-add chips** for common events (rest break, nap, walk, meeting,
  supplement, + more) drop an event at the current time. Expandable for
  duration / intensity / note only if wanted.
